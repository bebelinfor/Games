...
import prova.web.snoopygame.databinding.ActivityMaquinaBinding
...
class MaquinaActivity : AppCompatActivity() {
    ...
    fun buttonClick(view: View) {
        ...
        buttonSelecionado.setBackgroundResource(R.drawable.snoopy)
        ...
        while (i < 9) {
            ...
            if (tabuleiro[rX][rY] != "Snoopy" && tabuleiro[rX][rY] != "Peanuts") {
                break
            }

            i++
        }
// Recebe posicao do peanuts(X)
        tabuleiro[rX][rY]="Peanuts"

        val posicao = rX*3 + rY

        when(posicao){
            0 -> {
                binding.buttonZero.setBackgroundResource(R.drawable.peanuts)
                binding.buttonZero.isEnabled = false
            }
            1 -> {
                binding.buttonUm.setBackgroundResource(R.drawable.peanuts)
                binding.buttonUm.isEnabled = false
            }
            2 -> {
                binding.buttonDois.setBackgroundResource(R.drawable.peanuts)
                binding.buttonDois.isEnabled = false
            }
            3 -> {
                binding.buttonTres.setBackgroundResource(R.drawable.peanuts)
                binding.buttonTres.isEnabled = false
            }
            4 -> {
                binding.buttonQuatro.setBackgroundResource(R.drawable.peanuts)
                binding.buttonQuatro.isEnabled = false
            }
            5 -> {
                binding.buttonCinco.setBackgroundResource(R.drawable.peanuts)
                binding.buttonCinco.isEnabled = false
            }
            6 -> {
                binding.buttonSeis.setBackgroundResource(R.drawable.peanuts)
                binding.buttonSeis.isEnabled = false
            }
            7 -> {
                binding.buttonSete.setBackgroundResource(R.drawable.peanuts)
                binding.buttonSete.isEnabled = false
            }
            8 -> {
                binding.buttonOito.setBackgroundResource(R.drawable.peanuts)
...
// Verifica o vencedor
    ...
    fun verificaVencedor(tabuleiro: Array<Array<String>>): String? {
        ...
        for (linha in tabuleiro) {
            for (valor in linha) {
                if(valor.equals("Snoopy")||valor.equals("Peanuts")){
                    empate++
                }
            }
        }
        //Se existem 8 jogadas e não há três letras iguais, houve um empate
        if(empate == 8){
            return "Empate"
        }
        // Nenhum vencedor
